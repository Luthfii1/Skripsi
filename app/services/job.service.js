...
const findHeaderLine = async(filePath) => {
...
const validateAndParseCSV = async (filePath) => {
  return new Promise(async (resolve, reject) => {
    try {
      // Find the header line and column count
      const { headerLine, maxColumns, headerColumns } = await findHeaderLine(filePath);
      const records = [];
      const failedRecords = [];
...
const processChunkWithRetry = async (chunk, jobId, filename, startTime, totalRecords, processedRecords, initialCount) => {
  let retries = 0;
...
const processFileInChunks = async (filePath, jobId, filename) => {
...
const processMultipleFiles = async (files) => {
  ...
  for (const file of files) {
    ...
    processFileInChunks(file.path, job.id, job.filename)
      .catch(error => {
