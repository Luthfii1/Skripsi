 ...
 */
const getFailedRecordsByJob = async (jobId) => {
 ...
 */
const generateFailedRecordsCSV = async (jobId) => {
  const failedRecords = await db.failedUpload.findAll({
    where: { job_id: jobId },
    attributes: [
      'row_number',
 ...
 */
const updateFailedRecord = async (jobId, recordId, updateData) => {
 ...
 */
const processAllFailedRecords = async (jobId, records) => {
...
module.exports = {
  getFailedRecordsByJob,
  generateFailedRecordsCSV,
  updateFailedRecord,
  processAllFailedRecords
};
